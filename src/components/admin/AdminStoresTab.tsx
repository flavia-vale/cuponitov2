...
import { Plus, Save, Trash2, Pencil, X, Upload, Image as ImageIcon } from 'lucide-react';
import type { StoreBrand } from '@/lib/storeBranding';

interface Props { stores: StoreBrand[] | undefined; refetchStores: () => void; }
interface StoreForm { display_name: string; slug: string; icon_emoji: string; brand_color: string; fallback_color: string; logo_url: string; }
const emptyForm: StoreForm = { display_name: '', slug: '', icon_emoji: '🏷️', brand_color: '#575ecf', fallback_color: '#575ecf', logo_url: '' };
...
export function AdminStoresTab({ stores, refetchStores }: Props) {
  ...
  const uploadLogo = async (file: File) => {
    ...
    const { error } = await supabase.storage
      .from('blog-images') // Reusing the existing bucket for simplicity, or you can create a 'store-logos' one
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            <div className="space-y-3 lg:col-span-2">
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                <div><label className="mb-1 block text-xs font-medium text-muted-foreground">Emoji (Fallback)</label><Input value={form.icon_emoji} onChange={(e) => setForm((p) => ({ ...p, icon_emoji: e.target.value }))} placeholder="🏷️" /></div>
            ...
            <div className="space-y-3">
              <label className="block text-xs font-medium text-muted-foreground">Logo Oficial</label>
              {form.logo_url ? (
              ...
              <Input value={form.logo_url} onChange={(e) => setForm(p => ({ ...p, logo_url: e.target.value }))} placeholder="Ou cole a URL da logo" className="text-xs" />
            </div>
          </div>
          
          <div className="mt-6 flex gap-2">
            <Button onClick={handleSave} className="gap-1.5"><Save className="h-4 w-4" /> {editingId ? 'Atualizar Loja' : 'Cadastrar Loja'}</Button>
                ...
                <div key={store.id} className="flex items-center justify-between rounded-xl border border-border p-3 bg-card shadow-sm">
                  <div className="flex items-center gap-3">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg border bg-muted/30 overflow-hidden">
                      {store.logo_url ? (
                        <img src={store.logo_url} alt={store.display_name} className="h-full w-full object-contain p-1" />
                      ) : (
                        <span className="text-xl">{store.icon_emoji}</span>
                      )}
                    </div>
                    <div className="min-w-0">
                      <p className="truncate text-sm font-bold text-foreground">{store.display_name}</p>
                  ...
                  <div className="flex gap-1">
                    ...
                    <Button variant="ghost" size="icon" onClick={() => handleDelete(store.id)} className="h-8 w-8 text-destructive hover:text-destructive"><Trash2 className="h-3.5 w-3.5" /></Button>
                  </div>
                </div>
              ))}
            </div>
