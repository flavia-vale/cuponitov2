...
import { Copy, ExternalLink, CheckCircle2, Clock, Users, ShieldCheck } from 'lucide-react';
    ...
    <div className={cn(
        ...
        <div className="coupon-action p-4 sm:p-5 sm:pl-0 flex flex-col items-center sm:items-end justify-center gap-3 sm:min-w-[200px] border-t sm:border-t-0 border-black/5">
          {isCode ? (
            <div className="w-full space-y-2">
              <div className="code-box flex items-center gap-0 border-2 border-dashed border-[#FF4D00] rounded-xl overflow-hidden group/code transition-colors hover:border-[#FF4D00]/70">
                ...
                <button 
                  ...
                  className={cn(
                    "code-copy-btn h-full px-4 text-[10px] font-bold text-white transition-colors",
                    copied ? "bg-[#2E7D32]" : "bg-[#FF4D00] hover:bg-[#D83C00]"
                  )}
                >
      ...
      <div className="coupon-how bg-[#f9f7f4] border-t border-[#e8e5e0] p-3 hidden sm:group-hover:block transition-all animate-in slide-in-from-top-1 duration-200">
        <p className="text-[11px] text-[#888] leading-relaxed flex items-center gap-2">
          ...
          Como usar: {isCode ? "Copie o código e cole no campo 'Cupom de Desconto' ao finalizar sua compra." : "Clique no botão para ativar a oferta e ser redirecionado para a página com o desconto já aplicado."}
        </p>
      </div>
    </div>
  );
