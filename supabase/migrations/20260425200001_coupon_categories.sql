-- 1. Tabela de categorias (com IF NOT EXISTS)
...
-- 2. Seed: categorias (com ON CONFLICT para não dar erro se já existirem)
...
-- 3. RLS: leitura pública, escrita somente autenticado
ALTER TABLE public.coupon_categories ENABLE ROW LEVEL SECURITY;

-- Limpa políticas antigas antes de criar
DROP POLICY IF EXISTS "coupon_categories_read_public" ON public.coupon_categories;
DROP POLICY IF EXISTS "coupon_categories_write_admin" ON public.coupon_categories;
...
-- 4. Índices (com IF NOT EXISTS)
CREATE INDEX IF NOT EXISTS idx_coupon_categories_sort  ON public.coupon_categories(sort_order);
CREATE INDEX IF NOT EXISTS idx_coupon_categories_slug  ON public.coupon_categories(slug);